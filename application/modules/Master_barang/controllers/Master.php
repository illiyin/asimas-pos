...
class Master extends MX_Controller {
    ...
    function index(){
        ...
        $data['list_supplier'] = json_encode($this->Barangmodel->select($dataSelect, 'm_supplier_bahan', 'nama')->result());
        $data['list_satuan'] = json_encode($this->Barangmodel->select($dataSelect, 'm_satuan', 'nama')->result());
        $data['list_gudang'] = json_encode($this->Barangmodel->select($dataSelect, 'm_gudang', 'nama')->result());
        $data['list_kategori'] = json_encode($this->Barangmodel->select($dataSelect, 'm_bahan_kategori', 'nama')->result());

        $data['list_warna'] = json_encode($this->Barangmodel->select($dataSelect, 'm_bahan_warna', 'nama')->result());
        $data['list_det_warna'] = json_encode($this->Barangmodel->get('m_bahan_det_warna')->result());

        $data['list'] = json_encode($this->Barangmodel->select($dataSelect, 'm_bahan')->result());
		//echo $data;
		//print_r($data);
    ...
    function data(){
        ...
        foreach ($query->result_array() as $row) {
            $foto_url = base_url()."/upload/master_barang/placeholder.png";
            if(!empty($row["foto"])) {
                if(file_exists(URL_UPLOAD."/master_barang/".$row["foto"])) {
                    $foto_url = base_url()."/upload/master_barang/".$row["foto"];
                }
            }
            $nestedData     =   array();
