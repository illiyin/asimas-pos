...
class Master extends MX_Controller {
    ...
    function index(){
        ...
        $data['list_prov'] = json_encode($this->Suppliermodel->select($dataSelect, 'm_provinsi', 'nama')->result());
        $data['list_kota'] = json_encode($this->Suppliermodel->select($dataSelect, 'm_kota', 'nama')->result());
    	$data['list'] = json_encode($this->Suppliermodel->select($dataSelect, 'm_supplier_bahan', 'date_add', 'DESC')->result());
		//echo $data;
		//print_r($data);
	...
	function test(){
		...
		$dataSelect['deleted'] = 1;
		$list = $this->Suppliermodel->select($dataSelect, 'm_supplier_bahan', 'date_add', 'DESC')->result();
    ...
    function add(){
		...
		$checkData = $this->Suppliermodel->select($dataInsert, 'm_supplier_bahan');
		if($checkData->num_rows() < 1){
			$insert = $this->Suppliermodel->insert($dataInsert, 'm_supplier_bahan');
			if($insert){
				$dataSelect['deleted'] = 1;
				$list = $this->Suppliermodel->select($dataSelect, 'm_supplier_bahan', 'date_add', 'DESC')->result();
				echo json_encode(array('status' => 3,'list' => $list));
	...
	function get($id = null){
    	if($id != null){
    		$dataSelect['id'] = $id;
    		$selectData = $this->Suppliermodel->select($dataSelect, 'm_supplier_bahan');
    ...
    function edit(){
		...
		$checkData = $this->Suppliermodel->select($dataCondition, 'm_supplier_bahan');
		if($checkData->num_rows() > 0){
			$update = $this->Suppliermodel->update($dataCondition, $dataUpdate, 'm_supplier_bahan');
			if($update){
				$dataSelect['deleted'] = 1;
				$list = $this->Suppliermodel->select($dataSelect, 'm_supplier_bahan', 'date_add', 'DESC')->result();
				echo json_encode(array('status' => '3','list' => $list));
    ...
    function get_kota(){
    	...
    	$dataSelect['deleted'] = 1;
    	echo json_encode($this->Suppliermodel->select($dataSelect, 'm_kota', 'nama')->result());
    }
