...
class Master extends MX_Controller {
    ...
    function index(){
        ...
        $data['list_supplier'] = json_encode($this->Kategoribahanmodel->select($dataSelect, 'm_supplier_bahan', 'nama')->result());
        $data['list_satuan'] = json_encode($this->Kategoribahanmodel->select($dataSelect, 'm_satuan', 'nama')->result());
        $data['list_gudang'] = json_encode($this->Kategoribahanmodel->select($dataSelect, 'm_gudang', 'nama')->result());
        $data['list_kategori'] = json_encode($this->Kategoribahanmodel->select($dataSelect, 'm_bahan_kategori', 'nama')->result());

        $data['list_warna'] = json_encode($this->Kategoribahanmodel->select($dataSelect, 'm_bahan_warna', 'nama')->result());
        $data['list_det_warna'] = json_encode($this->Kategoribahanmodel->get('m_bahan_det_warna')->result());

        $data['list'] = json_encode($this->Kategoribahanmodel->select($dataSelect, 'm_bahan')->result());
		//echo $data;
		//print_r($data);
    ...
    function add(){
        ...
        $id = (!empty($params['id'])) ? $params['id'] : $this->Kategoribahanmodel->get_last_id("m_bahan") + 1;
        ...
        $checkData = $this->Kategoribahanmodel->select($dataInsert, 'm_bahan');
        if($checkData->num_rows() < 1){
            $insert = $this->Kategoribahanmodel->insert_id($dataInsert, 'm_bahan');
            if($insert){
                if(isset($params['id_warna'])){
                    $this->insert_detail($insert, $params['id_warna'], "warna");
                }
                $dataSelect['deleted'] = 1;
                $list = $this->Kategoribahanmodel->select($dataSelect, 'm_bahan')->result();
                $list_det_warna = $this->Kategoribahanmodel->get('m_bahan_det_warna')->result();
	...
	function get($id = null){
    	if($id != null){
    		$dataSelect['id'] = $id;
    		$selectData = $this->Kategoribahanmodel->select($dataSelect, 'm_bahan');
    ...
    function last_id() {
        echo "<script>console.log(".$this->Kategoribahanmodel->get_last_id("m_bahan").");</script>";
    }
    function edit(){
		$params = $this->input->post();
        $id = (!empty($params['id'])) ? $params['id'] : $this->Kategoribahanmodel->get_last_id("m_bahan") + 1;
		...
		$checkData = $this->Kategoribahanmodel->select($dataCondition, 'm_bahan');
		if($checkData->num_rows() > 0){
			$update = $this->Kategoribahanmodel->update($dataCondition, $dataUpdate, 'm_bahan');
            if(isset($params['id_warna'])){
    ...
    private function insert_detail($id_bahan, $data, $table) {
        ...
        if(!empty($table) AND !empty($data)) {
            ...
            $checkData = $this->Kategoribahanmodel->select($dataCondition, 'm_bahan_det_'.$table);
            if($checkData->num_rows() > 0) {
                //Delete old data first
                $this->Kategoribahanmodel->delete($dataCondition, 'm_bahan_det_'.$table);
            }
            ...
            $this->Kategoribahanmodel->insert_batch($dataInsert, 'm_bahan_det_'.$table);
        }
    }
