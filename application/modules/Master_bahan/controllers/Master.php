...
class Master extends MX_Controller {
    ...
    function index(){
        ...
        $data['list_supplier'] = json_encode($this->Bahanmodel->select($dataSelect, 'm_supplier_bahan', 'nama')->result());
        $data['list_satuan'] = json_encode($this->Bahanmodel->select($dataSelect, 'm_satuan', 'nama')->result());
        $data['list_gudang'] = json_encode($this->Bahanmodel->select($dataSelect, 'm_gudang', 'nama')->result());
        $data['list_kategori'] = json_encode($this->Bahanmodel->select($dataSelect, 'm_bahan_kategori', 'nama')->result());

        $data['list_warna'] = json_encode($this->Bahanmodel->select($dataSelect, 'm_bahan_warna', 'nama')->result());
        $data['list_det_warna'] = json_encode($this->Bahanmodel->get('m_bahan_det_warna')->result());

        $data['list'] = json_encode($this->Bahanmodel->select($dataSelect, 'm_bahan')->result());
		//echo $data;
		//print_r($data);
    ...
    function add(){
        ...
        $id = (!empty($params['id'])) ? $params['id'] : $this->Bahanmodel->get_last_id("m_bahan") + 1;
        ...
        $checkData = $this->Bahanmodel->select($dataInsert, 'm_bahan');
        if($checkData->num_rows() < 1){
            ...
            if($insert){
                ...
                $dataSelect['deleted'] = 1;
                $list = $this->Bahanmodel->select($dataSelect, 'm_bahan')->result();
                $list_det_warna = $this->Bahanmodel->get('m_bahan_det_warna')->result();
	...
	function get($id = null){
    	if($id != null){
    		$dataSelect['id'] = $id;
    		$selectData = $this->Bahanmodel->select($dataSelect, 'm_bahan');
    ...
    function last_id() {
        echo "<script>console.log(".$this->Bahanmodel->get_last_id("m_bahan").");</script>";
    }
    function edit(){
		$params = $this->input->post();
        $id = (!empty($params['id'])) ? $params['id'] : $this->Bahanmodel->get_last_id("m_bahan") + 1;
    ...
    private function insert_detail($id_bahan, $data, $table) {
        ...
        if(!empty($table) AND !empty($data)) {
            ...
            $checkData = $this->Bahanmodel->select($dataCondition, 'm_bahan_det_'.$table);
            if($checkData->num_rows() > 0) {
                //Delete old data first
                $this->Bahanmodel->delete($dataCondition, 'm_bahan_det_'.$table);
            }
            ...
            $this->Bahanmodel->insert_batch($dataInsert, 'm_bahan_det_'.$table);
        }
    }
